...
class WikiLinkCommand extends commando.Command {
  constructor( client ) {
    super( client, {
      name: 'wlink',
      group: 'wiki',
      memberName: 'wlink',
      description: 'Creates a link to a wiki page for as requested.'
    } );
  }
  ...
  async run( message, args ) {
    const strWiki = encodeURI( args )
      ...
      .replace( /\(/g, '%28' )
      .replace( /\)/g, '%29' );
    var msgWait = await message.channel.send( 'You can find the page for **' + args + '** at <' + wikiArticlePath + strWiki + '>' );
//    remove( message, msgWait );
  }  
}

module.exports = WikiLinkCommand;